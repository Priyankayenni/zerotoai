...
function getLocalProgress() {
  try {
    return new Set<string>(JSON.parse(localStorage.getItem(LOCAL_PROGRESS_KEY) || "[]"));
  } catch {
...
export default function App() {
  ...
  // Firebase auth state -> profile + progress
  useEffect(() => {
    ...
    setSessionUserId(auth.currentUser?.uid ?? null);

    const unsubscribe = auth.onAuthStateChanged(s => {
      setSessionUserId(s?.uid ?? null);
    });
  ...
  async function loadUserData(userId: string) {
    ...
    const [p, phaseIds] = await Promise.all([
      loadProfile(userId),
      loadCompletedPhaseIds(userId),
    ]);
    if (p) setProfile(p);
  ...
  async function refreshLeaderboard() {
    if (!isFirebaseConfigured || !sessionUserId) {
      setLeaders(addLocalLeader(fallbackLeaders));
      return;
    }
    try {
      const nextRaw = await loadLeaderboard({ limit: 10 });
      const next: Leader[] = nextRaw.map(r => ({
        id: r.id,
        name: r.name,
        class: null,
        completed: r.completed,
      }));
      setLeaders(next.length ? next : addLocalLeader(fallbackLeaders));
    } catch {
      setLeaders(addLocalLeader(fallbackLeaders));
    }
  }
  ...
  function addLocalLeader(base: Leader[]) {
    if (!profile || !sessionUserId) return base;
  ...
  async function handleAuth(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!isFirebaseConfigured || !auth) {
        ...
        setProfile(local);
        showToast("Local profile created! Connect Firebase to sync across devices.", "info");
        return;
      }

      if (authMode === "signup") {
        await authSignUp({ email, password, name, className: className || null });
  ...
  async function togglePhase(phase: Phase) {
    const adding = !completed.has(phase.id);
    const next = new Set(completed);
    if (adding) next.add(phase.id);
    ...
    if (!isFirebaseConfigured || !sessionUserId) return;
    await togglePhaseDone({ userId: sessionUserId, phaseId: phase.id, adding });
  }
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#08080c] text-stone-100">
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5">
          ...
          {profile ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={signOut} className="text-stone-400 transition hover:text-white" title="Sign out">
                <LogOut size={17} />
              </button>
            </div>
          ) : (
      ...
      <section id="dashboard" className="mx-auto max-w-7xl px-5 py-24">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {/* progress card */}
          <motion.div
            className="col-span-1 rounded-[2rem] border border-white/10 bg-white/4 p-6 backdrop-blur"
            whileHover={{ y: -4 }}
          ...
          >
            ...
            <div className="relative mb-4 h-3 overflow-hidden rounded-full bg-white/8">
              <motion.div
                className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-violet-300 via-fuchsia-300 to-emerald-300"
                initial={{ width: 0 }}
            ...
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="text-sm text-stone-400">
                  {completed.size} / {phases.length} phases
                </p>
              </div>
          ...
          {/* track breakdown */}
          <motion.div
            className="col-span-1 rounded-[2rem] border border-white/10 bg-white/4 p-6"
          ...
          {/* focus now */}
          <motion.div
            className="col-span-1 rounded-[2rem] border border-violet-300/20 bg-violet-300/6 p-6"
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {nextPhase.focus.slice(0, 3).map(f => (
                <span key={f} className="rounded-full border border-white/10 px-2.5 py-0.5 text-xs text-stone-300">
                  {f}
                </span>
              ))}
            </div>
            <button
              onClick={() => setShowPhaseModal(nextPhase)}
        ...
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap gap-2">
            {(["All", ...trackOrder] as const).map(t => (
              <button
                key={t}
                onClick={() => setFilter(t as Track | "All")}
            ...
            {filteredPhases.length ? (
              filteredPhases.map(p => (
                <PhaseRow
                  ...
                  onToggle={() => void togglePhase(p)}
                  onExpand={() => setShowPhaseModal(p)}
                />
              ))
            ) : (
              <motion.p
          ...
          <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/4">
            <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
              ...
              <span className="rounded-full bg-emerald-300/10 px-3 py-1 text-xs font-bold text-emerald-300">
                {isFirebaseConfigured && sessionUserId ? "Live" : "Demo"}
              </span>
            </div>
            {leaders.map((l, i) => (
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_1.1fr] lg:items-start">
          ...
          <form onSubmit={handleAuth} className="rounded-[2rem] border border-white/10 bg-white/4 p-7">
            <div className="mb-6 flex rounded-full border border-white/10 p-1">
              {(["signup", "signin"] as const).map(m => (
                <button
                  ...
                  className={`flex-1 rounded-full px-4 py-2 text-sm font-black capitalize transition ${
                    authMode === m ? "bg-white text-black" : "text-stone-400 hover:text-white"
                  }`}
                >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <AuthInput name="email" type="email" label="Email" placeholder="you@college.edu" required={Boolean(isFirebaseConfigured)} />
              <AuthInput name="password" type="password" label="Password" placeholder="Min 6 characters" required={Boolean(isFirebaseConfigured)} />
            </div>
            ...
            <p
              className={`mt-4 min-h-5 text-sm ${authMessage.includes("error") || authMessage.includes("wrong") ? "text-red-400" : "text-stone-400"}`}
            >
              {authMessage || (isFirebaseConfigured ? "✓ Firebase connected — progress syncs across devices." : "Running in prototype mode — progress saves locally.")}
            </p>
          </form>
        </div>
      </section>
      ...
      <footer className="border-t border-white/8 px-5 py-10 text-center text-sm text-stone-600">
        ...
        <br />
        <span className="text-xs opacity-50">React + Vite · TypeScript · Firebase · Framer Motion · Deploy on Vercel</span>
      </footer>
      ...
      <AnimatePresence>
        {showPhaseModal && (
          ...
          >
            ...
            <motion.div
              className="relative w-full max-w-lg rounded-[2rem] border border-white/12 bg-[#111118] p-7 shadow-2xl"
            ...
            >
              <button
                onClick={() => setShowPhaseModal(null)}
                aria-label="Close phase modal"
                className="absolute right-5 top-5 text-stone-500 hover:text-white"
              >
                <X size={20} />
              </button>
              <span className={`inline-block rounded-full border px-3 py-1 text-xs font-bold ${TRACK_BADGE[showPhaseModal.track] ?? ""}`}>{showPhaseModal.track}</span>
              ...
              {showPhaseModal.resource && <p className="mt-4 text-sm text-sky-300">📖 Resource: {showPhaseModal.resource}</p>}

              <button
                onClick={() => {
                  void togglePhase(showPhaseModal);
                  setShowPhaseModal(null);
                }}
                className={`mt-6 w-full rounded-full py-3 font-black transition ${
                  completed.has(showPhaseModal.id)
                    ? "bg-emerald-300 text-black hover:bg-emerald-200"
                    : "bg-violet-300 text-black hover:bg-white"
                }`}
              >
...
function LeaderRow({ leader, rank, isMe, total }: { leader: Leader; rank: number; isMe: boolean; total: number }) {
  ...
  return (
    <div
      className={`grid grid-cols-[44px_1fr_80px_60px] items-center gap-4 border-b border-white/8 px-6 py-4 last:border-b-0 ${
        isMe ? "bg-violet-300/5" : ""
      }`}
    >
      <div className="font-display text-xl font-black text-stone-500">{medals[rank - 1] ?? rank}</div>
      <div>
        <p className={`font-bold ${isMe ? "text-violet-200" : "text-white"}`}>
          {leader.name}{isMe && <span className="ml-1 text-xs text-violet-300">(you)</span>}
        </p>
      </div>
...
function AuthInput({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  return (
    <label className="block">
      ...
      <input
        {...props}
        className="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white outline-none transition placeholder:text-stone-600 focus:border-violet-300"
      />
    </label>
  );
}

function LaunchCard({ icon, step, title, text, color }: { icon: ReactNode; step: string; title: string; text: string; color: string }) {
  return (
    <motion.div
